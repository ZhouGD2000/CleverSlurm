...
def test_cscancel_calls_fake_scancel_and_records_note(isolated_home, fake_bin):
    from cslurm.db import connect, init_db
    from cslurm.cli.cscancel import cancel_job
    ...
    with connect() as conn:
        ...
        conn.execute(
            ...
            "values ('123456', 't', '/tmp', 'csbatch job.slurm', 'RUNNING', 't', 't')"
        )
...
def test_cscancel_passes_scancel_options_and_removes_note(isolated_home, fake_bin, tmp_path):
    from cslurm.cli.cscancel import cancel_job
...
def test_cscancel_module_entrypoint_forwards_output(isolated_home, fake_bin):
    write_executable(
        fake_bin / "scancel",
        "#!/bin/sh\nprintf 'cancelled %s\\n' \"$1\"\n",
    )
    ...
    result = subprocess.run(
        [sys.executable, "-m", "cslurm.cli.cscancel", "123456"],
        text=True,
