# CleverSlurm Feishu Notification and Failure-Analysis Guide

This document extends the CleverSlurm job-tracking system with Feishu notifications, deterministic failure classification, keyword log scanning, and AI-assisted semantic log analysis.
...
```text
~/.cslurm/config.toml
project-local .cslurm.yml
csbatch --success-criteria criteria.yml job.slurm
```
...
```text
[CleverSlurm][FAILED] Job 123456: DMFT U=4 Broyden run
...
```text
[CleverSlurm][SUSPICIOUS] Job 123456 completed but did not converge
...
```text
[CleverSlurm] Parameter scan summary: 100 jobs finished
...
Query:
cjobs group scan-2026-05-20-dmft-U-sweep
```
...
```text
[CleverSlurm] 43 failures detected in 5 minutes
...
cjobs failures --since "5 min ago"
```
...
For `csbatch`, generate `submission_batch_id` at submit time. For arrays, store parent array id and task id separately.
