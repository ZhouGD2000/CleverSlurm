...
class ModelClient:
    def __init__(
        ...
        *,
        ...
        self.format = (format or ai_format()).strip().lower()
        if self.format not in {OPENAI_COMPATIBLE, ANTHROPIC_COMPATIBLE}:
            raise RuntimeError(
                "Unsupported AI format. Use 'openai' or 'anthropic'."
            )
        self.api_key = api_key or ai_api_key()
        if not self.api_key:
            raise RuntimeError(
                "Missing AI API key. Set CSLURM_AI_API_KEY, configure [ai].api_key_env, or set [ai].api_key."
            )
        self.base_url = base_url or ai_base_url()
        if not self.base_url:
            raise RuntimeError("Missing AI base URL. Set CSLURM_AI_BASE_URL or [ai].base_url.")
        self.model = model or ai_model()
        if not self.model:
            raise RuntimeError("Missing AI model. Set CSLURM_AI_MODEL or [ai].model.")
    ...
    def _chat_json_once(self, messages: list[dict], *, model: str) -> str:
        if self.format == ANTHROPIC_COMPATIBLE:
