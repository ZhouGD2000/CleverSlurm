# AI-Slurm Job Tracking System: Coding Agent Implementation Guide
...
```text
aisbatch / aisrun / aisalloc / aiscancel / aiupdate
        |
        v
...
tracker daemon: squeue + sacct + sstat + logs
        |
        v
aijobs CLI / optional MCP server / optional Codex skill
```
...
```text
aisbatch      wraps sbatch
aisalloc      wraps salloc
aisrun        wraps srun
```
...
```text
aiscancel     wraps scancel
aihold        wraps scontrol hold
airelease     wraps scontrol release
airequeue     wraps scontrol requeue
aiupdate      wraps selected scontrol update operations
```
...
airequeue 123456 --note "Requeue after node failure."
aihold 123456 --note "Wait for previous DMFT run to finish."
```
...
Whenever AI suggests a file or interpretation, mark the source as `ai-suggested` and attach confidence.
...
5. Prepend ~/.ai-slurm/wrappers to PATH.
...
```text
~/.ai-slurm/wrappers/julia
~/.ai-slurm/wrappers/python
~/.ai-slurm/wrappers/matlab
~/.ai-slurm/wrappers/srun
~/.ai-slurm/wrappers/bash
```
...
```bash
AI_REAL_JULIA=/home/zgd/software/julia-1.11.5/bin/julia
AI_REAL_PYTHON=/home/zgd/miniconda/envs/env/bin/python
AI_REAL_MATLAB=/usr/local/MATLAB/R2024b/bin/matlab
AI_REAL_SRUN=/usr/bin/srun
```
...
```bash
aisbatch --trace-files job.slurm
```
...
aisbatch job.slurm
aisbatch --trace-files job.slurm
aisbatch --note "Testing DMFT peak stabilization with smaller mixing." job.slurm
```
...
```bash
aisrun -p CPU -c 16 julia --project=. test.jl
```
...
```bash
aiscancel 123456 --note "Wrong parameter U."
airequeue 123456 --note "Node failure; rerun."
aihold 123456 --note "Wait for previous result."
airelease 123456
aiupdate 123456 TimeLimit=08:00:00
```
...
aijobs recent
aijobs show 123456
aijobs logs 123456 --tail 200
aijobs files 123456
aijobs events 123456
aijobs ask "最近提交了什么任务？"
aijobs ask "哪些任务失败了，失败原因是什么？"
aijobs ask "最近有哪些 DMFT hybridization peak 相关任务？"
aijobs compare 123456 123890
```
...
[paths]
root = "~/.ai-slurm"
db = "~/.ai-slurm/db.sqlite"
store = "~/.ai-slurm/store"
...
```text
1. aisbatch records a job id from mocked sbatch --parsable output.
...
9. aiscancel writes CANCEL_REQUESTED event.
10. aijobs show returns job metadata and summary.
...
```text
aisbatch + SQLite + git snapshot + runtime command log + basic tracker
```
