...
def write_command_log(kind: str, real_executable: str, argv: list[str]) -> None:
    log_dir = Path(os.environ["AI_SLURM_LOG_DIR"])
    log_dir.mkdir(parents=True, exist_ok=True)
    record = {
        "time": _now(),
        "job_id": os.environ.get("AI_SLURM_JOB_ID", "unknown"),
...
def main() -> None:
    if len(sys.argv) < 2:
        raise SystemExit("usage: python -m ai_slurm.runtime.wrapper KIND [ARGS...]")
    kind = sys.argv[1]
    argv = sys.argv[2:]
    real = os.environ.get(f"AI_REAL_{kind.upper()}")
    if not real:
        raise SystemExit(f"missing AI_REAL_{kind.upper()}")
